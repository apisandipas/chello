...
function EmptyState({ showArchived }: { showArchived: boolean | undefined }) {
  return (
    <div className="flex flex-col items-center justify-center min-h-[60vh] text-center p-4">
      ...
      <h2 className="text-2xl font-bold mb-2">
        No {showArchived && "archived "}boards yet
      </h2>
      {showArchived ? (
        <Link
          from="/boards"
          ...
          className=""
        >
          <Button className="bg-cyan-700 hover:bg-cyan-800 mt-4">
            Show Unarchived Boards
          </Button>
        </Link>
      ) : (
        <>
          <p className="text-gray-600 mb-6 max-w-md">
            Get started by creating your first board. Boards help you organize
...
function RouteComponent() {
  ...
  if (!boards || boards.length === 0) {
    return <EmptyState showArchived={showArchived} />;
  }
        ...
        <div className="grid grid-cols-3 gap-4">
          {boards.map((board) => (
            <BoardListCard
              ...
              board={board as unknown as BoardWithCounts}
            />
          ))}
        </div>
      </div>
